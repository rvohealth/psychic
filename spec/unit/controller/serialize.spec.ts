...
import { Attribute, DreamApp, DreamSerializer, ViewModelSerializer } from '@rvoh/dream'
    ...
    context('without a serializer explicitly bound to the controller', () => {
      ...
      context('when the model is not a Dream', () => {
        const GreetSerializer = (data: Greeting) =>
          ViewModelSerializer(Greeting, data).customAttribute('greeting', data => `${data.word} world`, {
            openapi: 'string',
          })

        const GreetSerializer2 = (data: Greeting2) =>
          ViewModelSerializer(Greeting, data).customAttribute('greeting', data => `${data.word} goodbye`, {
            openapi: 'string',
          })
