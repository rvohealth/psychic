...
describe('when using a controller that implements cursorPagination', () => {
  beforeEach(async () => {
  ...
  it('correctly paginates results', async () => {
    ...
    const results = await request.get('/users/cursor-paginated', 200)
  ...
  context('cursorPagination', () => {
    ...
    context('query is sent with cursor param', () => {
      it('responds to page param', async () => {
        const cursor = (await User.order('createdAt').lastOrFail()).id.toString()
        const user = await User.create({ email: 'how@yadoin', password: 'howyadoin', name: 'fredo' })
        const results = await request.get('/users/cursor-paginated', 200, { query: { cursor } })
    ...
    context('request body is sent with cursor param', () => {
      it('responds to page param', async () => {
        const cursor = (await User.order('createdAt').lastOrFail()).id.toString()
        const user = await User.create({ email: 'how@yadoin', password: 'howyadoin', name: 'fredo' })
        const results = await request.post('/users/cursor-paginated-post', 200, { data: { cursor } })
